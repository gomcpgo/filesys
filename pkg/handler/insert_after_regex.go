...
func (h *FileSystemHandler) handleInsertAfterRegex(args map[string]interface{}) (*protocol.CallToolResponse, error) {
	...
	log.Printf("insert_after_regex - attempting to insert after occurrence %d of pattern '%s' in %s (autoIndent: %v, dry_run: %v, multiline: %v)",
	...
	var err error
	if multiline {
		newContent, err = search.InsertAfterRegexMultiline(path, pattern, contentToInsert, occurrence, autoIndent, true)
	} else {
