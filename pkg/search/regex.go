...
// InsertAfterRegexMultiline inserts content after a specific occurrence of a regex pattern in a file.
...
//   - The new content with insertions
//   - Error if any
func InsertAfterRegexMultiline(path, pattern, content string, occurrence int, autoIndent bool, multiline bool) (string, error) {
	// Read file content
	fileBytes, err := os.ReadFile(path)
	if err != nil {
		return "", fmt.Errorf("failed to read file: %w", err)
	...
	for i, idx := range indexes {
		...
		// If autoIndent is enabled, apply indentation from the current line
		if autoIndent {
			// Find the line containing the match end to extract its indentation
			lineStart := matchEnd
			for lineStart > 0 && fileContent[lineStart-1] != '\n' {
				lineStart--
			}
